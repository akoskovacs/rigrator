...
module Rigrator
  module Cli
    ...
    def self.drop_database
      url = Rigrator::DB.connection_url.to_s
    ...
    def self.set_database_to_schema(url)
      uri = URI.parse(url)
      if path = uri.path
        Rigrator::DB.connection_url = url.gsub(path, "/#{uri.scheme}")
        path.gsub("/", "")
      else
    ...
    def self.run_status
      Rigrator::DB.connect do |db|
        ...
        Log.info { "=======================================" }
        Rigrator.migration_status(db).each do |migration, migrated_at|
    ...
    def self.run_scaffold
      ...
      migration_file = Rigrator.create(ARGV.shift, Rigrator.migrations_dir, Time.local)
    ...
    def self.run_dbversion
      Rigrator::DB.connect do |db|
        begin
          Log.info { Rigrator.dbversion(db) }
        rescue
    ...
    def self.report_unordered_migrations(conflicting)
      ...
      Log.info { "
Rigrator will not run these migrations because they may have been written with an older database model in mind.
You should probably check if they need to be updated and rename them so they are considered a newer version." }
    end

    def self.print_help
      Log.info { "rigrator is a database migration management system for Crystal projects, *heavily* inspired by Goose (https://bitbucket.org/liamstask/goose/).

Usage:
    set DATABASE_URL environment variable i.e. export DATABASE_URL=postgres://user:pswd@host:port/database
    rigrator [options] <subcommand> [subcommand options]
...
Commands:
    ...
    def self.run
      ...
      begin
        ...
        Log.error(exception: e) { "Rigrator failed!" }
        exit 1
      end
    end
  end
